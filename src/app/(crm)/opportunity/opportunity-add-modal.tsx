...
import { Search, Edit, Copy, Plus, Settings, X, History, Trash2 } from "lucide-react";

type Tab = "files" | "tasks";

interface OpportunityAddForm {
  mainFields: any[];
...
interface OpportunityAddModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export function OpportunityAddModal({ isOpen, onClose }: OpportunityAddModalProps) {
  const [activeTab, setActiveTab] = useState<Tab>("files");
  const [productLines, setProductLines] = useState<ProductLine[]>([
    {
      id: "1",
      description: "Cleveland - Bag Mix n Fine",
      unit: "Salt",
      qty: 12,
      price: 250.0,
      discount: 15.0,
    ...
    {
      id: "2",
      description: "Hutchinson - Bulk Agriflow",
      unit: "Salt",
      qty: 12,
      price: 250.0,
      discount: 15.0,
  ...
  const subtotal = productLines.reduce((sum, line) => sum + line.total, 0);
  const totalCost = 5520.0;
  const totalProfit = 12345.0;
  const discountPercent = 0;
  const discountAmount = 520.0;
  const subtotal2 = 750.0;
  const invoiceTotal = 15750.0;
