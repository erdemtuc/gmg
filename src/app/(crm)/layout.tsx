...
function ClientLayoutWrapper({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="bg-brand-gray-50 min-h-screen w-full flex overflow-x-hidden">
      {/* Left Menu - 1/5 of viewport (20%) */}
      <aside className="bg-brand-primary-600 min-h-screen w-1/5 flex-shrink-0">
        <Suspense fallback={<LeftMenuSkeleton />}>
          <LeftMenuClientWrapper />
        </Suspense>
      </aside>

      {/* Main Content Area with Top Bar and Page Content - 4/5 of viewport (80%) */}
      <div className="flex flex-col flex-[4] min-w-0 min-h-0 overflow-x-hidden"> {/* flex-[4] takes 4/5 of remaining space (4 out of 5 total) */}
        {/* Top Bar */}
        <header className="h-14 w-full border-b border-gray-200 bg-white flex items-center px-6">
          <TopBar />
        </header>

        {/* Content and Side Pane container - Using flexbox to achieve 3/4-1/4 split of the 80% */}
        <div className="flex flex-1 overflow-hidden min-w-0">
          {/* Page Content - 3/5 of total viewport (3/4 of the 80%) */}
          <main className="bg-brand-gray-50 mt-6 mr-4 mb-9 ml-6 flex-[3] min-h-0 min-w-0 flex-1 overflow-y-auto"> {/* flex-[3] represents 3/4 of the 80% = 60% of total */}
            <ClientAppShell>
              {children}
            </ClientAppShell>
          </main>

          {/* Side Pane - Show different content based on page - 1/5 of total viewport (1/4 of the 80%) */}
          {!isCalendarPage && (
