import { ActivitySort, useActivityFilters } from './use-activity-filters';
import { ChevronUp, ChevronDown } from 'lucide-react';

interface ActivitySortComponentProps {
  showSort: boolean;
  onToggleSort: () => void;
}

export const ActivitySortComponent = ({ showSort, onToggleSort }: ActivitySortComponentProps) => {
  const { sort, setSortBy } = useActivityFilters();

  const handleFieldChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const newSort: ActivitySort = {
          ...
          <div className="p-2">
            ...
            <div className="flex gap-2">
              <button
                ...
                onClick={() => setSortBy({ field: sort?.field || 'title', direction: 'asc' })}
              >
                A-Z
              </button>
              <button
                ...
                onClick={() => setSortBy({ field: sort?.field || 'title', direction: 'desc' })}
              >
                Z-A
              </button>
            </div>
